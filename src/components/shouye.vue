<template>
  <div class="home">
    <div class="qrcode">
      <img
        src="data:image/png;base64,"
        alt="">
      <p>扫描二维码访问此项目前端地址</p>
      <p>或者点击<a href="https://gitee.com/joueyl/shopadmin">这里</a>直接打开地址</p>
    </div>
    <div class="project-address">
      <div>
        <div class="project-title">
          后台项目
        </div>
        <p><a href="https://github.com/macrozheng/mall">mall</a></p>
      </div>
      <div>
        <div class="project-title">
          前端项目
        </div>
        <p><a href="https://gitee.com/joueyl/shopadmin">joueyl-shopadmin</a></p>
      </div>
      <div>
        <div class="project-title">
          学习教程
        </div>
        <p><a href="https://github.com/macrozheng/mall-learning">mall-learning</a></p>
      </div>
    </div>
    <div class="project-data">
      <div>
        <img
          src="data:image/png;base64,"
          alt="">
        <div>
          <p>今日订单总数</p>
          <p>200</p>
        </div>
      </div>
      <div>
        <img
          src="data:image/png;base64,"
          alt="">
        <div>
          <p>今日销售总额</p>
          <p>￥5000.00</p>
        </div>
      </div>
      <div>
        <img
          src="data:image/png;base64,"
          alt="">
        <div>
          <p>昨日销售总额</p>
          <p>￥5000.00</p>
        </div>
      </div>
    </div>
    <div class="agency">
      <div class="agency-title">
        <p>待处理事务</p>
      </div>
      <div class="agency-active">
        <div>
          <span>待付款订单</span>
          <span>(10)</span>
        </div>
        <div>
          <span>已完成订单</span>
          <span>(10)</span>
        </div>
        <div>
          <span>待确认收货订单</span>
          <span>(10)</span>
        </div>
        <div>
          <span>待发货订单</span>
          <span>(10)</span>
        </div>
        <div>
          <span>新缺货登记</span>
          <span>(10)</span>
        </div>
        <div>
          <span>待处理退款申请</span>
          <span>(10)</span>
        </div>
        <div>
          <span>已发货订单</span>
          <span>(10)</span>
        </div>
        <div>
          <span>待处理退货订单</span>
          <span>(10)</span>
        </div>
        <div>
          <span>广告位即将到期</span>
          <span>(10)</span>
        </div>

      </div>
    </div>
    <div class="shop-user">
      <div>
        <div class="shop-user-title">
          <p>商品总览</p>
        </div>
        <div class="shop-user-active">
          <div>
            <p>100</p>
            <p>已下架</p>
          </div>
          <div>
            <p>400</p>
            <p>已上架</p>
          </div>
          <div>
            <p>50</p>
            <p>库存紧张</p>
          </div>
          <div>
            <p>500</p>
            <p>全部商品</p>
          </div>
        </div>
      </div>
      <div>
        <div class="shop-user-title">
          <p>用户总览</p>
        </div>
        <div class="shop-user-active">
          <div>
            <p>100</p>
            <p>今日新增</p>
          </div>
          <div>
            <p>200</p>
            <p>昨日新增</p>
          </div>
          <div>
            <p>100</p>
            <p>本月新增</p>
          </div>
          <div>
            <p>5000</p>
            <p>会员总数</p>
          </div>
        </div>
      </div>
    </div>
    <div class="statistics">
      <div class="statistics-title">
        <p>订单统计</p>
      </div>
      <div class="statistics-active">
        <div class="statistics-left">
          <div>
            <p>本月订单总数</p>
            <p>10000</p>
            <p><span>-10%</span>同比上周</p>
          </div>
          <div>
            <p>本月订单总数</p>
            <p>10000</p>
            <p><span>-10%</span>同比上周</p>
          </div>
          <div>
            <p>本月订单总数</p>
            <p>10000</p>
            <p><span>-10%</span>同比上周</p>
          </div>
          <div>
            <p>本月订单总数</p>
            <p>10000</p>
            <p><span>-10%</span>同比上周</p>
          </div>
        </div>
        <div id="statistics-right">
          <div class="date-sec">
            <el-date-picker v-model="value2" type="daterange" align="right" unlink-panels range-separator="至"
              start-placeholder="开始日期" end-placeholder="结束日期" :picker-options="pickerOptions" size="small" style="float: right;">
            </el-date-picker>
          </div>
          <div id="echarts"></div>
        </div>
      </div>
    </div>
  </div>
</template>
<script>
import * as echarts from 'echarts';
export default {
  data() {
      return {
        pickerOptions: {
          shortcuts: [{
            text: '最近一周',
            onClick(picker) {
              const end = new Date();
              const start = new Date();
              start.setTime(start.getTime() - 3600 * 1000 * 24 * 7);
              picker.$emit('pick', [start, end]);
            }
          }, {
            text: '最近一个月',
            onClick(picker) {
              const end = new Date();
              const start = new Date();
              start.setTime(start.getTime() - 3600 * 1000 * 24 * 30);
              picker.$emit('pick', [start, end]);
            }
          }, {
            text: '最近三个月',
            onClick(picker) {
              const end = new Date();
              const start = new Date();
              start.setTime(start.getTime() - 3600 * 1000 * 24 * 90);
              picker.$emit('pick', [start, end]);
            }
          }]
        },
        value1: '',
        value2: ''
      };
    },
  created() {
    // 注册必须的组件
    echarts.use([
      TitleComponent,
      TooltipComponent,
      GridComponent,
      DatasetComponent,
      TransformComponent,
      LineChart,
      LabelLayout,
      UniversalTransition,
      CanvasRenderer
    ]);
  },
  mounted() {
    var myChart = echarts.init(document.getElementById('echarts'));
    let option = {
      tooltip: {
        trigger: 'axis',
        axisPointer: {
          type: 'cross',
          label: {
            backgroundColor: '#6a7985'
          }
        }
      },
      toolbox: {
        feature: {
          saveAsImage: {}
        }
      },
      grid: {
        left: '3%',
        right: '4%',
        bottom: '3%',
        containLabel: true
      },
      xAxis: [
        {
          type: 'category',
          boundaryGap: false,
          data: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']
        }
      ],
      yAxis: [
        {
          type: 'value',
          data:[5,10,15,20]
        }
      ],
      series: [
        {
          name: 'Email',
          type: 'line',
          stack: 'Total',
          areaStyle: {},
          emphasis: {
            focus: 'series'
          },
          data: [120, 132, 101, 134, 90, 230, 210]
        },
        {
          name: 'Union Ads',
          type: 'line',
          stack: 'Total',
          areaStyle: {},
          emphasis: {
            focus: 'series'
          },
          data: [220, 182, 191, 234, 290, 330, 310]
        }
      ]
    };
    myChart.setOption(option);
  }

}
</script>
<style lang="less" scoped>
.home {
  box-sizing: border-box;
  padding: 60px 140px 20px 140px;
  position: relative;

  .statistics {
    width: 100%;
    height: 479px;
    margin-top: 20px;
    box-sizing: border-box;
    border: 1px solid #e6e6e6;

    .statistics-title {
      width: 100%;
      box-sizing: border-box;
      padding-left: 15px;
      background-color: #f2f6fc;

      p {
        line-height: 48px;
        font-weight: 700;
      }
    }

    .statistics-active {
      width: 100%;
      height: calc(100% - 48px);
      display: flex;

      .statistics-left {
        width: 200px;
        height: 100%;
        box-sizing: border-box;
        border-right: 1px solid #e6e6e6;
        padding: 20px;

        & div:nth-of-type(2n) {
          span {
            color: red !important;
          }
        }

        div {
          width: 100%;
          margin-bottom: 20px;

          & p:nth-of-type(1) {}

          & p:nth-of-type(2) {
            font-size: 26px;
          }

          & p:nth-of-type(3) {
            font-size: 14px;
            color: #999;

            span {
              color: rgb(11, 217, 11);
              margin-right: 5px;
            }
          }
        }
      }

      #statistics-right {
        flex: 1;
        box-sizing: border-box;
        padding: 10px;
        display: flex;
        flex-direction: column;
        .date-sec {
          width: 100%;
        }
        #echarts{
          flex: 1;
        }
      }
    }
  }

  .shop-user {
    width: 100%;
    height: 186px;
    margin-top: 20px;
    display: flex;
    justify-content: space-between;

    &>div {
      width: calc(100% / 2 - 10px);
      box-sizing: border-box;
      border: 1px solid #e6e6e6;

      .shop-user-title {
        height: 48px;
        background-color: #f2f6fc;
        box-sizing: border-box;
        line-height: 48px;
        padding-left: 15px;
        font-weight: 700;
      }

      .shop-user-active {
        padding: 40px;
        display: flex;

        div {
          width: 108px;

          & p:nth-last-of-type(2) {
            font-size: 24px;
            color: rgb(255, 101, 101);
            text-align: center;
          }

          & p:nth-last-of-type(1) {
            font-size: 16px;
            text-align: center;
          }
        }
      }
    }
  }

  .agency {
    width: 100%;
    height: 209px;
    box-sizing: border-box;
    border: 1px solid #e6e6e6;
    margin-top: 20px;

    .agency-title {
      height: 48px;
      width: 100%;
      background-color: #f2f6fc;
      box-sizing: border-box;
      padding-left: 15px;

      p {
        line-height: 48px;
        font-weight: 700;
      }
    }

    .agency-active {
      box-sizing: border-box;
      padding: 20px 40px;
      display: flex;
      flex-wrap: wrap;
      justify-content: space-between;

      div {
        width: calc(100% / 3 - 20px);
        height: 40px;
        border-bottom: 1px solid #e6e6e6;
        line-height: 40px;

        & span:nth-of-type(2) {
          color: red;
          float: right;
        }
      }
    }
  }

  .qrcode {
    position: absolute;
    width: 250px;
    height: 235px;
    top: 57px;
    right: 140px;
    box-shadow: 0 2px 12px 0 rgb(0 0 0 / 10%);
    box-sizing: border-box;
    padding: 20px;

    img {
      display: block;
      width: 150px;
      margin: auto;
    }

    p {
      text-align: center;
    }
  }

  .project-address {
    height: 110px;
    width: 100%;
    display: flex;

    &>div {
      height: 100%;
      width: calc((100% - 248px) / 3 - 20px);
      margin-right: 20px;
      box-sizing: border-box;
      border: 1px solid #e6e6e6;

      .project-title {
        // text-align: center;
        font-size: 16px;
        font-weight: 700;
        height: 48px;
        box-sizing: border-box;
        background-color: #f2f6fc;
        line-height: 48px;
        padding-left: 15px;
      }

      p {
        line-height: 62px;
        font-size: 18px;
        box-sizing: border-box;
        padding-left: 15px;
        font-weight: 500;
      }
    }
  }

  .project-data {
    width: 100%;
    height: 100px;
    display: flex;
    margin-top: 20px;

    &>div {
      height: 100%;
      width: calc((100% - 248px) / 3 - 20px);
      display: flex;
      align-items: center;
      margin-right: 20px;
      border: 1px solid #e6e6e6;
      padding: 20px;

      img {
        height: 100%;
      }

      div {
        flex: 1;
        box-sizing: border-box;
        padding-left: 15px;

        & p:nth-of-type(1) {
          font-size: 16px;
          color: #666;
        }

        & p:nth-of-type(2) {
          font-size: 18px;
          // color: #666;
        }
      }
    }
  }
}
</style>